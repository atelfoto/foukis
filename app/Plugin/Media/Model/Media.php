<?php
App::uses('AppModel', 'Model');//rajout philippe
class Media extends AppModel{
	...
	public function beforeDelete($cascade = true){
		...
		$resized = glob(WWW_ROOT.$info['dirname'].'/'.$info['filename'].'-*x*.jpg');//transformation philippe change '_' par '-'
	...
	public function beforeSave($options = array()){
		...
		if( isset($this->data['Media']['file']) && is_array($this->data['Media']['file']) && isset($this->data['Media']['ref']) ){
			...
			$filename 	= Inflector::humanize($pathinfo['filename'],'-');// voir 1°
			$search 	= array('/', '%id', '%mid', '%cid', '%y', '%m', '%f');
			$replace 	= array(DS, $ref_id, ceil($ref_id/1000), ceil($ref_id/100), date('Y'), date('m'), Inflector::humanize($filename));//1°change philippe slug par humanize
